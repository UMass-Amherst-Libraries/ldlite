...
queries = [
    ('agreements.entitlement', '/erm/entitlements', allrec),
   # ('agreements.erm_resource', '/erm/resource', allrec),
    ('agreements.org', '/erm/org', allrec),
    ('agreements.refdata_value', '/erm/refdata', allrec),
    ('agreements.sas', '/erm/sas', allrec, 2),
#    ('agreements.usage_data_provider', '/usage-data-providers', allrec),
...
for q in queries:
    try:
        if len(q) == 4:
            t = ld.query_erm(table=q[0], path=q[1], query=q[2], json_depth=q[3])
        else:
            t = ld.query_erm(table=q[0], path=q[1], query=q[2])
        tables += t
